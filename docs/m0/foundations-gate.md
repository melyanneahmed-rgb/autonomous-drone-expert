...
**لم تُدخل أي اعتمادية خارجية.** التثبيت النهائي يتم عبر `cargo-deny` لا بالثقة في جدول.

> **تحديث (ADR-0009، M1):** انتهت قاعدة «صفر اعتمادات مطلقة». تسمح بوابة العزل الآن
> باعتمادات **مسار داخلية فقط** إلى crates أعضاء في نفس الـworkspace (مع فحص احتواء المسار
> بنيويًا، ومطابقة اسم الحزمة، ورفض registry/git/URL/wildcard/hybrid/غير-العضو). **تبقى
> الاعتمادات الإنتاجية الخارجية ممنوعة**، و`cargo-deny 0.20.2` صار فحصًا مطلوبًا في CI
> (advisories/bans/licenses/sources). أول اعتماد خارجي يحتاج Dependency Audit وPR مستقلًا.
