...
const hostPath = path.join(webRoot, "src", "storage", "wasm-journal-host.mjs");
const glueRoot = path.resolve(
  process.argv[2] ?? path.join(repoRoot, "target", "storage-wasm-web"),
);
const glueJs = path.join(glueRoot, "ade_web_storage_wasm_bridge.js");
...
async function findPageTarget(devToolsUrl, pageUrl) {
  ...
  throw new Error("REAL_BROWSER_STORAGE_WASM_TARGET_UNAVAILABLE");
}
...
async function inspectPage(devToolsUrl, pageUrl) {
  ...
  try {
    ...
    throw new Error("REAL_BROWSER_STORAGE_WASM_TEST_TIMEOUT");
  } finally {
    socket.close();
  }
}
...
if (!browser) {
  console.error("REAL_BROWSER_STORAGE_WASM_ENVIRONMENT_UNAVAILABLE");
  process.exit(2);
}

const profile = await mkdtemp(path.join(os.tmpdir(), "ade-storage-wasm-smoke-"));
...
try {
  ...
  if (
    ...
    console.error(`REAL_BROWSER_STORAGE_WASM_FAILED (${result.output})`);
    console.error(`browser requests: ${servedRequests.join(", ")}`);
    console.error(result.stderr.slice(-4000));
    process.exitCode = 1;
  } else {
    console.log("real-browser Rust WASM + IndexedDB storage gate passed (A+B+C+D+E)");
  }
} finally {
